...
/* A cartoony, chibi-proportioned astronaut (~1.3 units tall, origin at the
   feet): oversized helmet with a big black glossy visor — shiny via a Phong
...
// the very shiny visor: SolarSystem feeds it a live CubeCamera envMap so it
// mirrors the actual scene around it. Deliberately Phong, not Physical:
// Standard/Physical materials route a cube envMap through a PMREM pre-filter
// that three.js reruns after every CubeCamera update — far too costly per
// frame. Phong samples the cube texture directly. Mix blends the mirror over
// a near-black base so it reads as dark glossy glass rather than chrome; a
// tight white specular lobe gives it the wet highlight.
const VISOR = new THREE.MeshPhongMaterial({
  ...
  // faint HUD light inside the glass; pulsed from the render loop
  emissive: '#1A4E5C',
});
const VISOR_EMIT_DIM = new THREE.Color('#0E2E38');
const VISOR_EMIT_HOT = new THREE.Color('#2A7A8C');

// high-tech trim: unlit cyan so the strips read as lit even in shadow. One
// shared material lets the render loop pulse every light at once; the
// antenna beacon has its own so it can blink out of step.
const GLOW = new THREE.MeshBasicMaterial({ color: '#6FE7FF', toneMapped: false });
const BEACON = new THREE.MeshBasicMaterial({ color: '#FF5C7A', toneMapped: false });
const GLOW_DIM = new THREE.Color('#2E7F92');
const GLOW_HOT = new THREE.Color('#9FF4FF');
const BEACON_DIM = new THREE.Color('#5A1F2A');
const BEACON_HOT = new THREE.Color('#FF7A94');
let haloTex = null;
const halos = [];
// soft additive puff behind a light so the glow bleeds onto the suit
function halo(size, x, y, z, color = '#6FE7FF') {
  haloTex ??= softDotTexture();
...
function ring(r, tube, matl, x, y, z, rotX = Math.PI / 2) {
  const m = new THREE.Mesh(new THREE.TorusGeometry(r, tube, 8, 24), matl);
  m.position.set(x, y, z);
  m.rotation.x = rotX;
  return m;
}
// slow breathing on the trim, a quicker heartbeat blink on the beacon
export function pulseAstronautLights(t) {
  const k = 0.5 + 0.5 * Math.sin(t * 2.2);
  GLOW.color.copy(GLOW_DIM).lerp(GLOW_HOT, k);
  const b = Math.pow(0.5 + 0.5 * Math.sin(t * 5.5), 6);
  BEACON.color.copy(BEACON_DIM).lerp(BEACON_HOT, b);
  for (const h of halos) {
    h.material.opacity = h.userData.beacon ? 0.15 + 0.7 * b
      : h.userData.visor ? 0.12 + 0.18 * k
      : 0.3 + 0.45 * k;
  }
  VISOR.emissive.copy(VISOR_EMIT_DIM).lerp(VISOR_EMIT_HOT, k);
}
...
function makeArm(x, y, rest) {
  ...
  elbow.add(ring(0.066, 0.012, GLOW, 0, -0.045, 0)); // lit wrist cuff
...
function makeLeg(x, y, rest) {
  ...
  knee.add(boot);
  knee.add(ring(0.072, 0.012, GLOW, 0, -0.1, 0)); // lit ankle cuff
...
export function buildAstronaut() {
  ...
  // glowing reactor core under the panel, with a soft bloom onto the chest
  const core = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.02, 20), GLOW);
  core.position.set(0, 0.52, 0.205);
  core.rotation.x = Math.PI / 2;
  body.add(core);
  body.add(ring(0.06, 0.008, GLOW, 0, 0.52, 0.205, 0));
  body.add(halo(0.32, 0, 0.52, 0.24));
  // light strip running inside the belt
  body.add(ring(0.185, 0.008, GLOW, 0, 0.45, 0));
  ...
  for (const tx of [-0.08, 0.08]) {
    ...
    body.add(tank);
    // lit caps on the tanks
    body.add(ring(0.045, 0.01, GLOW, tx, 0.835, -0.33));
  }
  // twin light bars down the back of the pack
  for (const bx of [-0.09, 0.09]) {
    const bar = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.22, 0.01), GLOW);
    bar.position.set(bx, 0.68, -0.315);
    body.add(bar);
  }
  ...
  body.add(visor);
  // wide, faint cyan bloom off the glass so the helmet reads as lit from within
  const visorHalo = halo(0.7, 0, 1.0, 0.3);
  visorHalo.userData.visor = true;
  visorHalo.material.opacity = 0.2;
  body.add(visorHalo);
  ...
  for (const px of [-0.29, 0.29]) {
    ...
    body.add(pod);
    // lit lens in the middle of each helmet pod
    const lens = ball(0.028, GLOW, px * 1.09, 1.0, 0);
    body.add(lens);
    body.add(halo(0.16, px * 1.25, 1.0, 0));
  }
  // a thin light seam around the helmet rim, just behind the gold ring
  body.add(ring(0.25, 0.006, GLOW, 0, 1.0, 0.17, 0));
  ...
  body.add(antenna);
  const tip = ball(0.025, BEACON, 0.235, 1.39, 0); // blinking beacon
  body.add(tip);
  const beaconHalo = halo(0.18, 0.235, 1.39, 0, '#FF5C7A');
  beaconHalo.userData.beacon = true;
  body.add(beaconHalo);
  ...
  return { group, body, limbs, swingArm: limbs[0], waveArm: limbs[1], outfits, visorMat: VISOR };
}
