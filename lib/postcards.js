...
export const PAGES = SYSTEMS.map((sys) => {
...
// how close (world units) the shuttle must fly to take the postcard:
// a couple of radii out, but never so tight that a tiny moon is a needle
export function collectRadius(body) {
  return Math.max(body.radius * 2.4, 2.2);
}
