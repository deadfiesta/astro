...
export function buildSecrets(scene) {
  ...
  SECRETS.forEach((s, i) => {
    ...
    // hidden until the formation intro reaches them (see revealSecrets)
    pivot.visible = false;
    ...
    out.push({
      ...
      // moments inside the intro's collapse (planets pop 0.85–1.7 s), out of list order
      revealAt: 0.95 + ((i * 7) % SECRETS.length) * 0.09,
      revealed: false,
...
/* grow the rocks in during the formation intro: `t` is the intro clock.
   Each condenses at its own moment among the planets with a quiet
   ease-out (no pop to give it away). Returns true once all are in. */
export function revealSecrets(secrets, t) {
  let all = true;
  for (const r of secrets) {
    if (r.revealed) continue;
    const k = (t - r.revealAt) / 0.6;
    if (k <= 0) { all = false; continue; }
    r.pivot.visible = true;
    if (k >= 1) { r.pivot.scale.setScalar(1); r.revealed = true; continue; }
    all = false;
    const e = 1 - Math.pow(1 - k, 3); // ease-out cubic, no overshoot
    r.pivot.scale.setScalar(Math.max(0.0001, e));
  }
  return all;
}
