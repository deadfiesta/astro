...
export function buildSecrets(scene) {
  ...
  SECRETS.forEach((s, i) => {
    ...
    // hidden until the system has formed, then eased in (see revealSecrets)
    pivot.visible = false;
    ...
    out.push({
      ...
      revealAt: 0.8 + ((i * 7) % SECRETS.length) * 0.55, // staggered, not in list order
      revealed: false,
...
/* fade the rocks in after the intro: `t` is seconds since the formation
   finished. Each one grows quietly at its own moment over ~6 s so no pop
   gives away where the secrets are. Returns true once all are in. */
export function revealSecrets(secrets, t) {
  let all = true;
  for (const r of secrets) {
    if (r.revealed) continue;
    const k = (t - r.revealAt) / 0.9;
