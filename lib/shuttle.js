...
/* A friendly space shuttle for fly mode. It is modelled ~10 units nose to
   tail and then shrunk by SHUTTLE_SCALE (about 3.3 units — a bit over an
   Earth and a half long, so it reads as a vehicle beside the planets rather
   than rivalling them). Nose points down -Z, matching
...
export function buildShuttle() {
  ...
  function shade(i, t, flick) {
    ...
    const fade = t < 0.42 ? 1 - t * 0.5 : Math.pow(1 - (t - 0.42) / 0.58, 1.3) * 0.79;
    col[i * 3] = r; col[i * 3 + 1] = g; col[i * 3 + 2] = b;
    alpha[i] = fade * flick;
    smoke[i] = sm;
    size[i] = (t < 0.42 ? 0.45 + t * 2.2 : 1.37 + (t - 0.42) * 3.6) * SHUTTLE_SCALE;
  }
  ...
  return {
    ...
    setThrust(t, time = 0) {
      ...
      for (let i = 0; i < throats.length; i++) {
        throats[i].material.opacity = thrust * (0.55 + Math.sin(time * 41 + i * 2.1) * 0.15);
      }
    ...
    updateExhaust(dt) {
      ...
      let toSpawn = Math.min(Math.floor(spawnDebt), 120);
      spawnDebt -= toSpawn;
      let scan = 0;
