...
// flame tints at idle (orange) and at full throttle (light blue)
const THROAT_HOT = new THREE.Color('#FFD9A3');
const THROAT_BLUE = new THREE.Color('#D6F1FF');
const GLOW_HOT = new THREE.Color('#FFA24A');
const GLOW_BLUE = new THREE.Color('#8CD6FF');
...
export function buildShuttle() {
  ...
  // As the throttle climbs the flame shifts hue toward a light-blue burn
  ...
  function shade(i, t, flick) {
    ...
    if (t < 0.12) {
      ...
      r2 = 0.92 - 0.2 * k; g2 = 0.98; b2 = 1;
    } else if (t < 0.42) {
      const k = (t - 0.12) / 0.3;
      r = 1; g = 0.85 - 0.4 * k; b = 0.45 - 0.35 * k; sm = 0;
      r2 = 0.72 - 0.35 * k; g2 = 0.9 - 0.15 * k; b2 = 1;
    } else {
      ...
      // blue burn cools into a paler, slightly blue-grey haze
      r2 = 0.37 - (0.37 - ash) * k; g2 = 0.75 - (0.75 - ash) * k; b2 = 1 - (1 - ash * 1.15) * k;
      sm = Math.sin(k * Math.PI) * 0.75;
    }
    r += (r2 - r) * blue; g += (g2 - g) * blue; b += (b2 - b) * blue;
