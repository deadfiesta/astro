...
// exhaust particle pool and feel
...
export function buildShuttle() {
  ...
  function shade(i, t, flick) {
    ...
    alpha[i] = fade * flick;
    smoke[i] = sm;
    size[i] = t < 0.42 ? 0.45 + t * 2.2 : 1.37 + (t - 0.42) * 3.6;
  }
  ...
  return {
    ...
    updateExhaust(dt) {
      ...
      while (toSpawn > 0 && scan < count) {
        ...
        const v = EXHAUST_SPEED * (0.7 + thrust * 0.6) * (0.85 + Math.random() * 0.3);
