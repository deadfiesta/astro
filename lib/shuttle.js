...
export function buildShuttle() {
  ...
  const parts = Array.from({ length: count }, () => ({
    // ix/iy/iz: the ship's velocity at spawn (kept — space has no drag);
    // vx/vy/vz: the jet's own velocity, which cools and slows
    age: 0, life: 0, ix: 0, iy: 0, iz: 0, vx: 0, vy: 0, vz: 0, seed: Math.random() * Math.PI * 2, drift: 1,
  }));
  ...
  return {
    ...
    updateExhaust(dt) {
      ...
      while (toSpawn > 0 && scan < count) {
        ...
        p.ix = shipVel.x; p.iy = shipVel.y; p.iz = shipVel.z;
        p.vx = tmpV.x * v;
        p.vy = tmpV.y * v;
        p.vz = tmpV.z * v;
      ...
      for (let i = 0; i < alive; i++) {
        ...
        // the jet slows as it cools (kids' physics: smoke lingers) and
        // wobbles more the older it gets — but only the jet part. The
        // inherited ship velocity is kept as-is: dragging it too made puffs
        // fall back toward the chase camera at ~30 u/s and paint over the
        // nose as they swept past the lens
        const drag = 1 - Math.min(0.9, (0.9 + t * 2.4) * dt);
        p.vx *= drag; p.vy *= drag; p.vz *= drag;
        const wob = t * t * 1.6 * SHUTTLE_SCALE * p.drift;
        pos[i * 3] += (p.ix + p.vx + Math.sin(now * 7 + p.seed) * wob) * dt;
        pos[i * 3 + 1] += (p.iy + p.vy + Math.cos(now * 6 + p.seed * 1.3) * wob) * dt;
        pos[i * 3 + 2] += (p.iz + p.vz + Math.sin(now * 5 + p.seed * 0.7) * wob) * dt;
