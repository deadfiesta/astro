...
export function buildShuttle() {
  ...
  // running lights. A lamp is a tiny unlit bulb plus an additive halo
  // sprite so its glow bleeds onto the hull; `pulseLights(t)` drives each
  // one's blink pattern every frame. Real aviation layout, kid-bright:
  // red port / green starboard at the wingtips, white strobes beside
  // them, red anti-collision beacons on the fin and belly, and a chase of
  // cyan panel lights down each flank
  const haloTex = softDotTexture();
  const lamps = [];
  function lamp(x, y, z, color, pattern, { size = 1.0, phase = 0, min = 0.1, max = 0.75, bulb: r = 0.11 } = {}) {
    const base = new THREE.Color(color);
    const bulb = mesh(new THREE.SphereGeometry(r, 10, 8), new THREE.MeshBasicMaterial({ color: base.clone(), toneMapped: false }), x, y, z);
    const glow = new THREE.Sprite(new THREE.SpriteMaterial({
      map: haloTex, color: base.clone(), transparent: true, opacity: max,
  ...
  lamp(-3.75, -0.15, 2.55, '#FF3B4E', 'steady', { size: 1.3 }); // port (left) — red
  lamp(3.75, -0.15, 2.55, '#3BFF7A', 'steady', { size: 1.3 }); // starboard (right) — green
  lamp(-3.55, -0.15, 2.85, '#FFFFFF', 'strobe', { size: 1.6, max: 0.9, bulb: 0.09 });
  lamp(3.55, -0.15, 2.85, '#FFFFFF', 'strobe', { size: 1.6, max: 0.9, bulb: 0.09, phase: 0.5 });
  lamp(0, 3.18, 2.1, '#FF4A5C', 'beacon', { size: 1.5, max: 0.85 }); // top of the fin
  lamp(0, -0.98, 0.3, '#FF4A5C', 'beacon', { size: 1.5, max: 0.85, phase: Math.PI }); // belly
  ...
  return {
    ...
    pulseLights(t) {
      for (const L of lamps) {
        let k;
        switch (L.pattern) {
          case 'strobe': { // a quick double flash every ~1.4 s
            const f = ((t * 0.7 + L.phase) % 1 + 1) % 1;
            k = f < 0.05 || (f > 0.13 && f < 0.18) ? 1 : 0;
            break;
          }
          case 'beacon': // sharp rotating-beacon sweep about once a second
            k = Math.pow(0.5 + 0.5 * Math.sin(t * 6.3 + L.phase), 5);
            break;
        ...
        L.bulb.material.color.copy(L.base).multiplyScalar(0.18 + 0.82 * k);
