...
export function buildShuttle() {
  ...
  // each one's pattern every frame. Red port / green starboard at the
  // wingtips with a soft white twinkle beside them, a cyan chase of panel
  ...
  lamp(-3.55, -0.15, 2.85, '#FFFFFF', 'twinkle', { size: 1.4, max: 0.8, bulb: 0.08 });
  lamp(3.55, -0.15, 2.85, '#FFFFFF', 'twinkle', { size: 1.4, max: 0.8, bulb: 0.08, phase: 1.7 });
  ...
  return {
    ...
    pulseLights(t) {
      for (const L of lamps) {
        let k;
        switch (L.pattern) {
          case 'twinkle': // a soft white sparkle, never fully dark
            k = 0.3 + 0.7 * Math.pow(0.5 + 0.5 * Math.sin(t * 3.1 + L.phase), 3);
            break;
